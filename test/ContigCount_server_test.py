...
from ContigCount.ContigCountImpl import ContigCount


class ContigCountTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'ContigCount',
        ...
        for nameval in config.items('ContigCount'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = ContigCount(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_ContigCount_" + str(suffix)
